...
class ConsoleWriter extends OutputStyle
{
    ...
    public function consoleOutput(string $line, $type)
    {
        if (config('installer.store.with_output')) {
            ($type === Process::ERR)
                ? $this->consoleLabeledLine('!', '| '.$line, 'warning')
                : $this->consoleLabeledLine('✓', '| '.$line, 'success');
        }
    }

    public function consoleLabeledLine(string $label, string $message, string $labelFormat = 'default', int $indentColumns = 0): void
    {
        render(<<<HTML
                <div class="ml-1">
                    <span class="{$this->textStyles[$labelFormat]}">{$label}</span>

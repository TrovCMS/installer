...
trait Debug
{
    ...
    protected function debugReport(): void
    {
        ...
        $this->consoleWriter->text([
            'The following is the configuration TrovCMS Installer has computed by merging:',
        ]);
        ...
        $savedConfig = [];
        if (File::isFile(config('home_dir').'/.trov/config')) {
            $savedConfig = Dotenv::createMutable(config('home_dir').'/.trov', 'config')->load();
        }
        $this->arrayToTable(
            $savedConfig,
            [
    ...
    {
        $config = Arr::prepend(config('installer.store'), config('home_dir'), 'home_dir');
        $this->arrayToTable($this->dotFlatten('installer.store', $config));
    }
